          ...
          <form class="flex flex-col gap-4" (ngSubmit)="submit()">
            ...
            <label class="flex flex-col gap-1 text-xs font-medium text-muted-foreground">
              <div class="flex items-center justify-between">
                <span>Password</span>
                <a routerLink="/forgot-password" class="font-medium text-primary hover:underline">Forgot password?</a>
              </div>
              <div class="flex items-center gap-2 rounded-lg border border-input bg-input px-3 focus-within:border-ring">
...
export class LoginComponent {
  ...
  async submitGoogle(idToken: string) {
    ...
    // authGuard reroutes a Google account that still needs a Primary Brand / phone to /choose-brand.
    const returnUrl = this.route.snapshot.queryParamMap.get('returnUrl') || '/dashboard';
    this.router.navigateByUrl(returnUrl);
  }
}
